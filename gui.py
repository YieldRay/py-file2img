...
class WinGUI(Tk):
    ...
    def __init__(self):
        ...
        self.widget_dic["tk_label_select_file_label"] = (
            self.__tk_label_select_file_label(self)
        )
        ...
        self.widget_dic["tk_label_select_image_label"] = (
            self.__tk_label_select_image_label(self)
        )
        self.widget_dic["tk_button_convert_image"] = self.__tk_button_convert_image(
            self
        )
    ...
    def __tk_label_lb1(self, parent):
        label = Label(
            parent, text="任意文件转换为图片", anchor="center", font=("微软雅黑", 16)
        )
    ...
    def __tk_label_lb2(self, parent):
        label = Label(
            parent, text="图片还原为文件", anchor="center", font=("微软雅黑", 16)
        )
...
class Win(WinGUI):
    ...
    def handler_convert_file(self, evt):
        ...
        path = filedialog.asksaveasfilename(
            title="选择保存的路径",
            initialfile="修改此处文件名_后缀要为png.png",
            filetypes=[("PNG图像", ".png")],
        )
        if path == "":
    ...
    def handler_convert_image(self, evt):
        ...
        file_path = lb["text"]
        path = filedialog.asksaveasfilename(
            title="选择保存的路径", initialfile="修改此处文件名及后缀"
        )
        if path == "":
            messagebox.showerror("失败", "未选择保存的路径")
        else:
    ...
    def __event_bind(self):
        self.widget_dic["tk_button_select_file"].bind(
            "<Button-1>", self.handler_select_file
        )
        self.widget_dic["tk_button_convert_file"].bind(
            "<Button-1>", self.handler_convert_file
        )
        self.widget_dic["tk_button_select_image"].bind(
            "<Button-1>", self.handler_select_image
        )
        self.widget_dic["tk_button_convert_image"].bind(
            "<Button-1>", self.handler_convert_image
        )
